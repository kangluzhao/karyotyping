...
library(tidyverse)
Sample <- read.table("test_data_IMB_QIS4_8_5X.txt",header = T,sep = "\t")
...
df <- calcValueByWindow(pos = Samplenew$Positioncum, value = Samplenew$IMB_QIS4_8_5X)

library(ggplot2)
library(RColorBrewer)
ggplot(Samplenew, aes(Positioncum,IMB_QIS4_8_5X)) + theme(panel.grid = element_blank(), axis.line = element_line(color = 'black'), panel.background = element_rect(fill = 'transparent')) + geom_point(aes(color = Chromosome),size = 0.3, show.legend = FALSE) + scale_color_manual(values = brewer.pal(12,"Set3")) + ylim(0,1) + labs(x = 'Chromosome', y = 'index', title = 'IMB_QIS4_8_5X') + scale_x_continuous(breaks = chrnum$center, labels = chrnum$Chromosome, expand = c(0, 0)) + geom_line(data = df, aes(x = pos, y = value), color = 'black', size = 0.5, show.legend = FALSE) + geom_hline(yintercept = c(0.2, 0.8), color = 'red', linetype = 2, size = 0.4)
ggsave('IMB_QIS4_8_5X.png',width = 9.6, height = 1.64)